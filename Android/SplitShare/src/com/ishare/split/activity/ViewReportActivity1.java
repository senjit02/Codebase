...
public class ViewReportActivity1 extends BaseActivity {
				...
				submitPres.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						if(null==mailContentMap || mailContentMap.isEmpty()){
							Toast toast = Toast.makeText(ViewReportActivity1.this, "Make atlest one selection",Toast.LENGTH_LONG);
							toast.show();
						}else{
							popupWindow.dismiss();
	...
	private Map<Integer, ExpenseUserVO> populateAllExpenseDetailForGroup(){
		...
		if(null!=cursor && cursor.moveToFirst()){
			...
			Toast toast = Toast.makeText(ViewReportActivity1.this, "No expense detail available", Toast.LENGTH_SHORT);
			toast.show();
		}
